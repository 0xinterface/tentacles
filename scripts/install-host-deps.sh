...
# 2.328 runner line until apt-cache resolves one.
pick_icu() {
    # shellcheck disable=SC1091
    . /etc/os-release 2>/dev/null || true
    case "${ID:-}${VERSION_ID:-}" in
        debian13* | ubuntu24* | ubuntu23*) first=libicu74 ;;
    ...
    for pkg in "$first" libicu74 libicu72 libicu70 libicu67 libicu66 libicu63; do
...
if ! LIBICU="$(pick_icu)"; then
    die "no libicu package found for this distro (tried: libicu74 libicu72 libicu70 libicu67 libicu66 libicu63)"
fi
